...
#if IS_ENABLED(CONFIG_ANALOG_INPUT_ADC_READ_HOOKS)
#include <zephyr/pm/device.h>   // zmk/zephyr/include/zephyr/pm/device.h

static inline void adc_read_before_hook(void) {
    const struct device *const spi0_dev = DEVICE_DT_GET(DT_NODELABEL(spi0));    // for 595s and pmw3610
    const struct device *const spi2_dev = DEVICE_DT_GET(DT_NODELABEL(spi2));    // for pcm320x

    pm_device_state_lock(spi0_dev);     // avoid resuming this spi device by pm

    pm_device_action_run(spi0_dev, PM_DEVICE_ACTION_SUSPEND);
    pm_device_action_run(spi2_dev, PM_DEVICE_ACTION_RESUME);
}

static inline void adc_read_after_hook(void) {
    const struct device *const spi0_dev = DEVICE_DT_GET(DT_NODELABEL(spi0));    // for 595s and pmw3610
    const struct device *const spi2_dev = DEVICE_DT_GET(DT_NODELABEL(spi2));    // for pcm320x
...
#endif  // CONFIG_ANALOG_INPUT_ADC_READ_HOOKS
